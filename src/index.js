...
const camo = {
	blender: async (request) => {
		return proxyPass(new URL("https://git.blender.org"), request);
	},
	archlinux: async (request) => {
		return proxyPass(new URL("https://geo.mirror.pkgbuild.com"), request);
	},
	archlinuxarm: async (request) => {
		return proxyPass(new URL("https://ca.us.mirror.archlinuxarm.org"), request);
	},
	manjaro: async (request) => {
		return proxyPass(new URL("http://manjaro.mirrors.uk2.net"), request, (content) => {
			return content.replaceAll("Port 80", "Port 443");
		});
	},
	alpine: async (request) => {
		return proxyPass(new URL("https://dl-cdn.alpinelinux.org"), request);
	},
	ubuntu: async (request) => {
		return proxyPass(new URL("http://archive.ubuntu.com"), request, (content) => {
			return content.replaceAll("Port 80", "Port 443");
		});
	},
	debian: async (request) => {
		return proxyPass(new URL("http://archive.debian.org"), request, (content) => {
			return content.replaceAll("Port 80", "Port 443");
		});
	},
	fedora: async (request) => {
		return proxyPass(new URL("https://archives.fedoraproject.org"), request);
	},
	centos: async (request) => {
		return proxyPass(new URL("https://mirrors.edge.kernel.org/centos"), request, (content) => {
			return content.replaceAll("Index of /centos", "Index of ");
		});
	},
	opensuse: async (request) => {
		return proxyPass(new URL("https://opensuse.mirror.iphh.net"), request);
	},
	freebsd: async (request) => {
		return proxyPass(new URL("https://mirrors.mit.edu/FreeBSD"), request, (content) => {
			return content.replaceAll("Index of /FreeBSD", "Index of ");
		});
	},
	kde: async (request) => {
		return proxyPass(new URL("https://mirrors.mit.edu/kde"), request, (content) => {
	...
	msys2: async (request) => {
		return proxyPass(new URL("https://cdimage.debian.org/mirror/msys2.org/"), request,
			(content) => {
	...
	netbsd: async (request) => {
		return proxyPass(new URL("https://cdimage.debian.org/mirror/netbsd.org/NetBSD/"), request,
			(content) => {
	...
	voidlinux: async (request) => {
		return proxyPass(new URL("https://cdimage.debian.org/mirror/voidlinux/"), request,
			(content) => {
...
async function proxyPass(proxyURL, request, modifyContentCallback = null, pathnameCallback = null) {
	...
	if (rsp.headers.get("Content-Type") !== null && rsp.headers.get("Content-Type").includes("text")) {
